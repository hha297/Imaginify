...
// GET ALL IMAGE
export async function getAllImages({ limit = 10, page = 1, searchQuery = '' }: { limit?: number; page: number; searchQuery?: string }) {
        try {
                await connectToDatabase();
                ...
                const resourceId = resources.map((resource: any) => resource.public_id);
